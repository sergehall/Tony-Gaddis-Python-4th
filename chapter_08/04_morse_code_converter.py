# 4. Конвертер азбуки Морзе. Азбука Морзе представляет собой кодировку,
# где каждая буква алфавита, каждая цифра и различные знаки препинания
# представлены серией точек и тире. В табл. 8.4 и 8.5 показана часть этой
# азбуки. Напишите программу, которая просит пользователя ввести строковое
# значение и затем преобразует это строковое значение в кодировку
# азбукой Морзе.
...
def converter(a):
    ...
    while index < len(a):
        ...
        print(morse[position],
              end=" ")  # give a space break after each ch in morse
        index += 1
